...
class EfficientTransformerDecoderLayer(AbstractDecoderLayer):
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(EfficientTransformerDecoderLayer, self).__init__()
        self.normalize_before = normalize_before
        
        mod = importlib.import_module('efficient_attention')
        self_attn_cls = getattr(mod, self_attention_type)
        cross_attn_cls = getattr(mod, cross_attention_type)
       
        print(self_attention_type, cross_attention_type)
        print(kwargs)
        
        self.self_attn = self_attn_cls(embed_dim=d_model, num_heads=nhead, dropout=attention_dropout, **kwargs)
        self.multihead_attn = cross_attn_cls(embed_dim=d_model, num_heads=nhead, dropout=attention_dropout)
